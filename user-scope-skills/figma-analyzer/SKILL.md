...
**필수 절차:**
1. Figma URL/파일 경로 확인 및 MCP 도구 가용 여부를 체크한다
2. 확인 결과를 **텍스트로 출력**한다
3. **반드시 STOP하고 사용자 응답을 기다린다**
4. 사용자가 응답한 **다음 턴**에서 AskUserQuestion으로 Phase 0을 시작한다
...
| "빨리 분석해줘"에 STOP 생략 | 로드 턴에서 AskUserQuestion 호출 | **플랫폼 제약은 우회 불가** — 급해도 URL 확인 후 STOP 필수 |
