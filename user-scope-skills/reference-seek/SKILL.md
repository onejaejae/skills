...
| Aspect | `/reference-seek` | `/google-search` | `/tech-decision` | `/deep-research` |
|--------|--------------------|--------------------|-------------------|-------------------|
| **Purpose** | Find reusable code patterns and repos | Real-time web search with full page content | A vs B decision analysis with recommendation | Multi-agent deep investigation with cited report |
| **Output** | Categorized references with code excerpts | Search result list (URLs + snippets/body) | Conclusion-first decision report | Comprehensive research report with confidence assessment |
| **Depth** | Medium -- quality-filtered repos + code dive | Shallow -- returns raw search results | Medium-Deep -- multi-source comparison | Deep -- parallel agents + cross-validation |
| **When to use** | Need implementation examples to learn from | Need quick search results or specific URLs | Need to choose between alternatives | Need thorough investigation of a broad topic |
| **Tools** | gh API, context7, WebSearch, Explore agent | chromux (real Chrome browser) | WebSearch, WebFetch, community sources | WebSearch, WebFetch, chromux, Gemini CLI, subagents |
