...
description: >
  ...
  <topic>.
validate_prompt: |
  Must contain Executive Summary section. Must contain Sources section with URLs. Must contain Confidence Assessment. Must have at least 3 distinct sources cited.
---
...
| Aspect | `/deep-research` | `/google-search` | `/reference-seek` | `/tech-decision` |
|--------|-------------------|--------------------|--------------------|-------------------|
| **Purpose** | Multi-agent deep investigation with cited report | Real-time web search with full page content | Find reusable code patterns and repos | A vs B decision analysis with recommendation |
| **Output** | Comprehensive research report with confidence assessment | Search result list (URLs + snippets/body) | Categorized references with code excerpts | Conclusion-first decision report |
| **Depth** | Deep -- parallel agents + cross-validation | Shallow -- returns raw search results | Medium -- quality-filtered repos + code dive | Medium-Deep -- multi-source comparison |
| **When to use** | Need thorough investigation of a broad topic | Need quick search results or specific URLs | Need implementation examples to learn from | Need to choose between alternatives |
| **Tools** | WebSearch, WebFetch, chromux, Gemini CLI, subagents | chromux (real Chrome browser) | gh API, context7, WebSearch, Explore agent | WebSearch, WebFetch, community sources |
