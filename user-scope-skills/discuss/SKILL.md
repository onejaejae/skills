...
validate_prompt: |
  ...
  Must include Maturity level (Exploratory/Forming/Solid) in synthesis.
  Must NOT generate PLAN.md, run git commands, or prescribe implementation.
  Must NOT contain code blocks with implementation.
  Must NOT exceed 10 turns total (hard cap).
---
...
- **Clarify requirements** → Say: `"Got it. Run clarify:vague to refine your requirements."` → Stop
...
### 2.1 Probe Direction (Auto-Select)
...
- **Failure scenarios** — How could this go wrong? What are the failure modes?
- **Counter-arguments** — What would someone argue against this?
- **Stress test** — Does this hold up under edge cases and scale?
- **Alternative paths** — What other approaches haven't we considered?
...
### 2.5 Hard Turn Cap (Absolute Maximum)
...
3. **No implementation** — Do not write code or prescribe specific implementation. If the user asks "how would you implement this?", you may describe a **high-level conceptual approach in 2-3 sentences** (no code, no file paths, no specific libraries). Then redirect back to probing: "Now that we've sketched an approach, what assumptions are we making?"
4. **No `AskUserQuestion` for probes** — Socratic questions go in natural language. Reserve `AskUserQuestion` for meta-decisions (next steps).
5. **Max 7 turns before synthesis offer** — Prevent endless discussion without capture.
6. **Turn 10 = forced synthesis** — Absolute maximum. No "Keep going" option after turn 10.
7. **"I don't know" is valid** — Capture it as an Open Question, never force an answer.
...
- [ ] No implementation prescribed (high-level only if explicitly requested, max 2-3 sentences)
- [ ] Turn 10 hard cap enforced (if applicable)
- [ ] Sibling skill suggested if user seemed to want /mirror, /stepback, or clarify:vague instead
