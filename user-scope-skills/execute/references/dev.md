...
```
# Guard: parse worker output safely
IF result is empty OR result is not valid JSON OR result.status is undefined:
  log_to_audit("Worker {task_id}: malformed output — could not parse result")
  IF {depth} == "quick":
    HALT with "Worker {task_id} returned unparseable output. No recovery in quick mode."
  ELSE:
    # Treat as FAILED — proceed to :Verify for independent assessment
