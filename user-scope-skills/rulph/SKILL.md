...
validate_prompt: |
  ...
  Must include circuit breaker logic with stagnation detection.
  Must include state file write for Stop hook integration.
  Must include cost estimate section.
  Must include differentiator table vs check/tribunal/council.
---
...
## When to Use (vs. Similar Skills)

| Skill | Purpose | Key Difference |
|-------|---------|----------------|
| `/rulph` | **Iterative rubric-scored improvement loop** | Builds custom rubric, multi-model scoring, autonomous improvement until threshold met |
| `/check` | Pre-push change verification | Rule-based checklist against `.dev/rules/`, no scoring, no improvement loop |
| `/tribunal` | 3-perspective adversarial review | Fixed Risk/Value/Feasibility axes, one-shot verdict (APPROVE/REVISE/REJECT), no iteration |
| `/council` | Multi-perspective deliberation | Team-based debate with tradeoff map, no scoring rubric, no autonomous improvement |

**Use `/rulph` when**: You have an artifact that needs measurable quality improvement through iterative refinement with a custom rubric. Use `/tribunal` for a quick pass/fail review. Use `/council` for open-ended deliberation where the answer is unclear. Use `/check` for pre-push validation against existing rules.

## Cost Estimate

This is an **expensive loop skill**. Budget before running:

| Component | Per Round | Max (5 rounds) |
|-----------|-----------|-----------------|
| Multi-model evaluation (3 models) | 3 LLM calls | 15 LLM calls |
| Improvement worker agent | 1 LLM call | 5 LLM calls |
| **Total** | **4 LLM calls** | **~20 LLM calls** |

Actual cost depends on artifact size. Large artifacts (>500 lines) multiply token usage per call. Consider setting a lower `max_rounds` (2-3) for cost-sensitive evaluations, or a higher threshold to exit earlier on high-quality artifacts.
...
**Degradation tiers**:
- **3 models AVAILABLE**: Full confidence. Standard averaging.
- **2 models AVAILABLE**: `REDUCED CONFIDENCE` flag. Standard averaging across 2 models. Log which model was SKIPPED/DEGRADED.
- **1 model AVAILABLE**: `LOW CONFIDENCE` flag. Single-model scores lack cross-validation. Divergence analysis is skipped (no second model to compare).
- **0 models AVAILABLE** (all external failed): Fall back to main agent self-evaluation (see Minimum model guarantee above). Flag as `SELF-EVAL ONLY`.
...
📊 Score: XX/100 (Codex: XX | Gemini: XX | Claude: XX) — Threshold: [threshold] · Floor: [floor]
   Confidence: [FULL / REDUCED CONFIDENCE / LOW CONFIDENCE / SELF-EVAL ONLY]
...
```
Bash: SESSION_ID="[session ID from UserPromptSubmit hook]" && hoyeon-cli session set --sid $SESSION_ID --json '{"rulph": {"round": [round], "max_rounds": 5, "score": [overall], "threshold": [threshold], "status": "active", "iteration": 0, "max_iterations": 15}}'
```

Replace `[round]`, `[overall]`, etc. with actual values. **Always preserve `max_rounds`, `iteration`, and `max_iterations`** — the Stop hook reads these fields to decide whether to allow exit. Omitting them causes the hook to fall back to defaults, which may not match the user's settings. `iteration` resets to 0 here — the Stop hook increments it each time it fires within a round, providing a per-round safety net.
...
After appending to `score_history` (round >= 2), check for stagnation patterns:

**Score regression**: If overall score dropped compared to previous round:
> "Warning: Score regression detected (round [N-1]: XX -> round [N]: XX). The improvement may have introduced side effects. Next improvement will target the regressed criterion specifically."

When regression is detected, the next improvement dispatch targets the criterion that regressed most, not the lowest-scoring criterion. This prevents oscillation where fixing one criterion breaks another.

**Plateau**: If the overall score changed by less than 2 points for 2 consecutive rounds:
> "Warning: Score plateau detected (rounds [N-2] through [N] within 2pts). Switching improvement strategy — the worker will receive expanded context including all prior failed suggestions to avoid repeating the same approach."

When plateau is detected, include the list of prior improvement suggestions in the worker prompt so it can try a different approach.

**Oscillation**: If a specific criterion's score alternates up/down across 3+ rounds (e.g., 60 -> 75 -> 62 -> 78):
> "Warning: Oscillation detected on '[criterion]'. Improvements to this criterion may conflict with other criteria. Consider the criterion as structurally coupled — improvement worker will receive constraint to preserve scores on related criteria."
