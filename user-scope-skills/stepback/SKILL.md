...
validate_prompt: |
  ...
  Output must be under 10 content lines (blank separator lines excluded).
...
If the user provided explicit text after `/stepback`, treat that as additional context about what they're working on.

**No-context fallback:** If the conversation history has no prior work context AND the user provided no text after `/stepback`, prefix your output with:
```
⚠ No work context detected — analysis is based on general patterns only.
```
Then produce the best analysis you can from whatever signal exists. This warning does NOT count toward the 10-line limit.
...
### Check 2: Side Effects / Ripple Impact
> "Could this change affect something else?"

For code work: identify potential blast radius — other modules, teams, users, or systems that could be affected. For non-code work (docs, design, process): identify downstream stakeholders, workflows, or artifacts that could be impacted. If no obvious effects, say "No visible side effects."
...
Total output: 4 content lines (1 step-back question + 3 checks) plus closing. Keep under 10 content lines — blank lines between sections do not count.
...
If the conversation was in Korean, use: **"계속하세요."** (not a literal translation — this is the canonical Korean closing).
...
5. **Max 10 content lines** (blank separators excluded) — If you can't fit it in 10, you're overexplaining. Cut.
