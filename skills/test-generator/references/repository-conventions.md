...
class Test{Feature}Repository:
    ...
    @pytest.fixture
    async def test_{entity}(self, {dependency}_repository, async_session):
        """테스트용 {entity} 생성"""
        entity = await {dependency}_repository.create(
            **create_kwargs,
...
@pytest.fixture
async def test_research(self, research_repository, async_session):
    """테스트용 연구 생성"""
    research = await research_repository.create(
        title="테스트 연구",
        project_id=None,
        created_by_id=1,
        session=async_session,
