...
name: plan-generator
description: >
  Task Definition이 준비되었고 구현 계획 수립이 필요할 때 사용합니다.
---
...
- **테스트 포함**: 구현 Step에 해당하는 테스트를 같은 Step 또는 별도 Step으로 포함
...
- [ ] 모든 Step에 필수 필드(파일, 내용, 테스트, 커밋, 수용 기준)가 포함되었는가?
- [ ] 모든 AC가 최소 1개 Step의 수용 기준에 매핑되었는가? (Step 4 자체 검증)
