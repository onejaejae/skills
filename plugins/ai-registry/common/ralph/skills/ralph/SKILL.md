...
|-------|---------|-------------|
| **/ralph** | DoD-based iterative loop with Stop-hook re-injection + independent verification | Single task, need verified-done guarantee, no spec.json |
| /execute | Spec-driven orchestrator: parallel worker dispatch by spec.json | You have a spec.json with structured tasks |
| /rulph | Lightweight iterative loop (no DoD verification, simpler) | Quick repetitive task, no verification needed |
| /bugfix | Root-cause diagnosis + spec generation + /execute | Bug report needing investigation before fix |
...
| Error | Recovery |
|-------|----------|
| Session ID missing ($CLAUDE_SESSION_ID unset) | Warn user: "Ralph loop requires session hooks. Run from Claude Code CLI." HALT |
| `hoyeon-cli session set` fails | Retry once; if still fails, warn user and proceed without Stop hook (single-pass mode) |
| DoD file not writable (permission/path) | Fallback to `/tmp/ralph-dod-{timestamp}.md`, update state path |
| ralph-verifier agent fails to spawn | Log error, attempt direct verification (lose context isolation), warn user |
| Circuit breaker reached (max iterations) | Print remaining unchecked DoD items, save DoD file path, HALT |
